import { EmployeeModel } from './../services/employee.service.js';
...
getEmployeesByBranchId = async (req, res) => {
  ...
  try {
    const employees = await this.employeeModel.getEmployeesByBranchId(branchId);
    res.json(employees);
  } catch (error) {
    res.status(500).json({ error: 'Внутренняя ошибка сервера' });
    console.error(error);
...
updateEmployeeById = async (req, res) => {
  const employeeId = req.params.id;
  try {
    const updatedRows = await this.employeeModel.updateEmployeeById(employeeId, req.body);
    if (updatedRows === 0) {
      res.status(404).json({ error: 'Сотрудник не найден' });
    } else {
...
deleteEmployeeById = async (req, res) => {
  const employeeId = req.params.id;
  try {
    const deletedRows = await this.employeeModel.deleteEmployeeById(employeeId);
    if (deletedRows === 0) {
      res.status(404).json({ error: 'Сотрудник не найден' });
    } else {
