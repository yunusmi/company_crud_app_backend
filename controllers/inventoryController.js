...
export class InventoryController {
  ...
  createInventoryItem = async (req, res) => {
    try {
      ...
      const inventoryId = await this.inventoryModel.createInventoryItem(product_id, quantity_in_stock);
      res.status(201).json({ message: 'Запись инвентаря создана', inventoryId });
    } catch (error) {
  ...
  getInventoryItemById = async (req, res) => {
    ...
    try {
      const inventoryItem = await this.inventoryModel.getInventoryItemById(inventoryId);
  ...
  updateInventoryItemById = async (req, res) => {
    ...
    try {
      ...
      const updatedRows = await this.inventoryModel.updateInventoryItemById(inventoryId, quantity_in_stock);
      if (updatedRows === 0) {
        res.status(404).json({ error: 'Запись инвентаря не найдена' });
      } else {
  ...
  deleteInventoryItemById = async (req, res) => {
    const inventoryId = req.params.id;
    try {
      const deletedRows = await this.inventoryModel.deleteInventoryItemById(inventoryId);
      if (deletedRows === 0) {
        res.status(404).json({ error: 'Запись инвентаря не найдена' });
      } else {
